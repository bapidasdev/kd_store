...
const Cart = () => {
  const { cartItems, products, navigate, cartCount, totalCartAmount, removeFromCart, updateCartItem, axios, user, setCartItems } = useContext(AppContext);
  ...
  const orderViaWhatsApp = async () => {
    try {
      ...
      const orderDetails = cartArray
        .map((item) => `${item.name} (x${item.quantity}) - ₹${item.offerPrice * item.quantity}`)
        .join("%0A"); // new line

      const total = totalCartAmount() + (totalCartAmount() * 2) / 100;

      const message = `Hello KD Fresh Vegetables,%0A` +
        `I would like to place the following order:%0A%0A` +
        `${orderDetails}%0A%0A` +
        `Total Amount: ₹${total}%0A` +
        `Delivery Address: ${selectedAddress ? `Contect No :-${selectedAddress.phone}, ${selectedAddress.street}, ${selectedAddress.state}, ${selectedAddress.country}` : "Not provided"}%0A` +
    ...
    <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
            ...
            {showAddress && (
              <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                ...
                <p
                  onClick={() => {
                    if (!user) {
                      toast.error("Please login before adding an address");
                    } else {
        ...
        <button
          ...
          className="w-full py-3 mt-3 cursor-pointer bg-green-500 text-white font-medium hover:bg-green-600 transition flex items-center justify-center gap-2"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" width="22" height="22">
            <path d="M12 0C5.373 0 0 4.943 0 11.057c0 2.183.658 4.281 1.903 6.089L0 24l7.125-1.873A12.13 12.13 0 0012 22.114c6.627 0 12-4.943 12-11.057C24 4.943 18.627 0 12 0zm0 20.057c-1.579 0-3.132-.36-4.525-1.043l-.324-.163-4.233 1.115 1.13-4.104-.211-.336A8.902 8.902 0 013.1 11.06C3.1 6.475 7.048 2.8 12 2.8c4.952 0 8.9 3.675 8.9 8.26 0 4.586-3.948 8.26-8.9 8.26z" />
          </svg>
          Order via WhatsApp
        </button>
      </div>
